	...
	<div id="Container">
			...
			<div id="logo">
				...
				<table style="margin-left: 660px;width: 700px">
					<tr>
						用户名:${resul==null}
						<c:if test="${resul}">
						<td width="100px"><a>欢迎:</a></td>
						<td width="100px"><a style="color:orange;font-size: 20px;">${result}</a></td></c:if>
						
						<c:if test="${result==null}">
						<td width="100px"><a href="view/login.jsp">登录</a></td>
						<td width="100px"><a href="view/registr.jsp">注册</a></td>
						</c:if>
		...
		<div id="Content">
			...
			<div id="Content-Main">
				<!-- 网页的右面部分-->
				<center>
				<form action="login/shoppingCart.do" method="post">
				...
				<tr><td>
				请输入需要购买的数量:<input type="text" name="items_num" value="1"/><br>
				商品ID:<input type="text" name="str" value='<c:out value="${it.id}"/>'/>
				</td>
				</tr>
				<tr>
				<td><input type="submit" value="加入购物车"/></td>
				
				<td><a href='login/purchase.do?id=<c:out value="${it.id}"/>'>立即购买</a></td>
				</tr>
				</c:forEach>
				</table>
				</form>
